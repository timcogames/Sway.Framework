...
#include "vertexelementsemantic.h"
...
void VertexAttributeBinding::mergeLayout() {
	...
	for (u32 i = 0; i < _vertexDeclaration.get()->getElementCount(); ++i) {
		...
		VertexAttributeUmap_t::iterator iter = _attributes.find(getAttributeNameBySemantic(vertexElement.semantic));
		if (iter != _attributes.end()) {
			...
			case kVertexElementSemantic_Position:
				...
				attribute.offset = BUFFER_OFFSET(_vertexLayoutOffset);
...
void VertexAttributeBinding::setVertexInputAttributeDescription(const std::string &attributeName, int size, u32 dataType, bool normalized) {
	...
	VertexAttributeUmap_t::iterator iter = _attributes.find(attributeName);
	if (iter == _attributes.end())
		return;
	...
	attribute.offset = BUFFER_OFFSET(_vertexAttributeOffset);
...
void VertexAttributeBinding::enable(u32 byteStride) {
	BOOST_FOREACH(VertexAttribute &attribute, _attributes | boost::adaptors::map_values) { 
		Extensions::glVertexAttribPointerARB(attribute.location, attribute.componentCount, TypeUtils::toGL(attribute.dataType), attribute.normalized, byteStride, attribute.offset);
...
void VertexAttributeBinding::setAttributes(VertexAttributeUmap_t attributes) {
	_attributes = attributes;
}

VertexAttributeUmap_t VertexAttributeBinding::getAttributes() {
	return _attributes;
}
...
std::string VertexAttributeBinding::getAttributeNameBySemantic(u32 semantic) const {
	switch (semantic) {
